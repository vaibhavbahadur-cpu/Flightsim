export class FlightTelemetry {
    constructor() {
        ...
        // Add Blink Animation for Stall
        const style = document.createElement('style');
        style.innerHTML = `
            @keyframes hudBlink { 0% { opacity: 1; } 50% { opacity: 0; } 100% { opacity: 1; } }
            .stall-alert { color: #ff3300; font-size: 24px; text-align: center; font-weight: bold; animation: hudBlink 0.4s infinite; margin-bottom: 5px; }
        `;
    ...
    update(data, controls) {
        ...
        this.container.innerHTML = `
            ${stallHUD}
            <div style="text-align:center; border-bottom:1px solid #00ff44; margin-bottom:10px; font-size: 16px;">B747-8 SYSTEMS</div>
            <div style="display: flex; justify-content: space-between;"><span>AIRSPEED:</span><span>${knots} KTS</span></div>
            ...
            <div style="display: flex; justify-content: space-between;"><span>VERT SPD:</span><span>${vsi > 0 ? '+' : ''}${vsi} FPM</span></div>
            <hr style="border:0; border-top:1px dashed #00ff44; margin: 10px 0;">
            <div style="display: flex; justify-content: space-between;"><span>PITCH:</span><span>${data.pitch.toFixed(1)}°</span></div>
            <div style="display: flex; justify-content: space-between;"><span>ROLL:</span><span>${data.roll.toFixed(1)}°</span></div>
            <div style="margin-top:12px;">
                <div style="font-size: 10px; margin-bottom: 3px;">THRUST: ${throttlePercent}%</div>
